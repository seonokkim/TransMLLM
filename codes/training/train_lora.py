"""
LoRA Fine-tuning Script for TransMLLM

Supports parameter-efficient fine-tuning using LoRA adapters.
"""
...
from codes.model.lora_adapter import setup_lora
...
def train_lora(
    ...
    run_name: str = "transmllm_lora",
    **kwargs
) -> None:
    """
    Train TransMLLM with LoRA.
    ...
    # Setup LoRA
    model.model = setup_lora(
        model.model,
        rank=config["model"]["lora_rank"],
        alpha=config["model"]["lora_alpha"],
        target_modules=config["model"]["lora_target_modules"],
        use_4bit=config.get("model", {}).get("use_4bit", False),
    )
    ...
    # Train
    print(f"Starting LoRA training with {len(train_dataset)} samples")
    if val_dataset:
...
def main() -> None:
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Train TransMLLM with LoRA")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
